...
- The build output must be constant from build to build, so avoid random elements or build timestamps in the output. You can still include variable content, but it should be deterministic based on other site content. See [`Origami.randomFrom`](https://weborigami.org/builtins/origami/randomFrom.html) for a discussion of how to incorporate random output in a stable way.
